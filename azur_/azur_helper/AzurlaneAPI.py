...
async def format_data_into_html(data):
    ...
    if 'retrofitHullType' in data:
        ...
        retrofit_soup.find(id='avatar').img.replace_with(retrofit_soup.new_tag("img", src="images/Texture2D/" + pinyin(
            ...
            '''
        retrofit_soup.find(id='avatar').img.replace_with(retrofit_soup.new_tag("img", src="images/Texture2D/" + str(
            data['id']) + '_g.png'))
...
def get_equipment_recommendation():
    url = "https://wiki.biligame.com/blhx/%E8%A3%85%E5%A4%87%E4%B8%80%E5%9B%BE%E6%A6%9C"
    response = requests.get(url, headers=header)
    soup = BeautifulSoup(response.text, "lxml")
    div_list = soup.find(class_='noresize').find("img").get("src")
    return div_list
