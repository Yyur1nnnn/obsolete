...
cancel_download_image = on_command("取消碧蓝图片更新",
                                   priority=5,
                                   block=True,
...
async def _():
    if get_update_state("download_image"):
        kill_popen()
        await cancel_download_image.finish("以取消当前图片更新", at_sender=True)
    else:
        await cancel_download_image.finish("现在还没有图片更新任务呢！", at_sender=True)
